...
class ProductProduct(models.Model):
    ...
    def get_products_based_on_movement_date_ept(self, from_datetime, company):
        ...
        qry = ("""select distinct product_id from stock_move where exported_in_woo= '%s'""" % (True))
        logger.info("Common qry: %s", qry)
        self._cr.execute(qry)
        result += self._cr.dictfetchall()
        product_ids = [product_id.get('product_id') for product_id in result]
        product_ids = self.env['stock.move'].search([('exported_in_woo', '=',True)]).mapped('product_id.id')
        logger.info("Common product_ids: %s", product_ids)
    ...
    def get_onhand_qty_ept(self, warehouse, product_list):
        ...
        if simple_product_list_ids:
            ...
            result = self._cr.dictfetchall()
            for i in result:
                qty_stock=self.env['product.product'].search([('id','=',i.get('product_id'))],limit=1)
                logger.info("result: %s", qty_stock.qty_available)
                onhand_qty.update({i.get('product_id'): qty_stock.qty_available})
        return onhand_qty
