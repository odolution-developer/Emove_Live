...
class WooInstanceConfig(models.TransientModel):
    ...
    def woo_create_analytic_plan_account(self, woo_instance):
        ...
        if self.env.user.has_group('analytic.group_analytic_accounting') and woo_instance:
            ...
            analytic_account_id = self.woo_search_or_create_analytic_account(woo_instance, analytic_account_plan)
    ...
    def woo_search_or_create_analytic_account(self, woo_instance, analytic_account_plan):
        """
        ...
        analytic_account = analytic_account_obj.search([('name', '=', woo_instance.name),
                                                        ('plan_id', '=', analytic_account_plan.id),
                                                        ('company_id', '=', woo_instance.company_id.id)], limit=1)
        if not analytic_account:
            analytic_account = analytic_account_obj.create({'name': woo_instance.name, 'plan_id': analytic_account_plan.id})
            analytic_account.write({'company_id': woo_instance.company_id.id})
...
class ResConfigSettings(models.TransientModel):
    ...
    def onchange_woo_instance_id(self):
        ...
        if instance:
            ...
            self.woo_credit_note_register_payment = instance.woo_credit_note_register_payment or False
            self.woo_credit_note_payment_journal = instance.woo_credit_note_payment_journal or False
    ...
    def execute(self):
        ...
        if instance:
            ...
            values['woo_credit_note_register_payment'] = self.woo_credit_note_register_payment or False
            values['woo_credit_note_payment_journal'] = self.woo_credit_note_payment_journal or False
