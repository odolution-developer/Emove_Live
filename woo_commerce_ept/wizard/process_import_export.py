...
class WooProcessImportExport(models.TransientModel):
    ...
    auto_apply_adjustments = fields.Boolean(default=True, help="If it is set, the quants will be applied "
    ...
    def prepare_data_and_import_stock(self):
        ...
        woo_products = woo_product.search([('default_code','!=',''),('exported_in_woo', '=', True), ('woo_instance_id', '=', instance.id)])
        sku = woo_products.mapped('default_code')
        logger.info("SKU to import %s" % (sku))
    ...
    def request_for_import_stock(self, sku_chunk, instance, product_fields, log_lines):
        ...
        try:
            ...
            #logger.info("Product Response %s" % (product_response))
    ...
    def prepare_data_for_inventory_adjustment(self, woo_products, res_product, duplicate_woo_product, products_stock,
        ...
        if product:
            if (res_product.get('manage_stock') and
                    res_product.get('stock_quantity') is not None and \
                ...
                if product.product_id.id not in duplicate_woo_product:
                    ...
                    _logger.info("Duplicate product found in WooCommerce store with SKU 1: %s ", product.default_code)


        else:
            woo_variants= self.env["woo.product.product.ept"].search([]).filtered(lambda x: int(x.woo_template_id.woo_tmpl_id) == res_product.get('id'))
            for rec in woo_variants:
                product = woo_products.filtered(lambda x: int(x.variant_id) == int(rec.variant_id))
                if product:
                    if (res_product.get('manage_stock') and
                            res_product.get('stock_quantity') is not None and \
                            product.product_id.detailed_type == 'product'):
                        if product.product_id.id not in duplicate_woo_product:
                            ...
                            _logger.info("Duplicate product found in WooCommerce store with SKU 2: %s ",
                                         product.default_code)

                else:
                    message="Import Stock for product %s does not exist in odoo woo product id %s" % (res_product.get('sku'),res_product.get('id'))
                    error = self.env["common.log.lines.ept"].search([("message",'=',message)],limit=1)
                    if not error:
                        log_line_id = self.env["common.log.lines.ept"].create_common_log_line_ept(
