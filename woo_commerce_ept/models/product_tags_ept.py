...
class WooTagsEpt(models.Model):
    ...
    woo_instance_id = fields.Many2one("woo.instance.ept", "Instance", required=True, copy=False)
    ...
    def woo_export_product_tags(self, instances, woo_product_tags):
        ...
        for instance in instances:
            ...
            product_tags = []
            woo_tags = woo_product_tags.filtered(lambda x: x.woo_instance_id == instance)
            woo_tag_batches = self.env["woo.product.template.ept"].prepare_batches(woo_tags)
            for woo_tag_batche in woo_tag_batches:
                for woo_product_tag in woo_tag_batche:
