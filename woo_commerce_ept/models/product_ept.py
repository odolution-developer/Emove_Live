...
class WooProductTemplateEpt(models.Model):
    ...
    def search_odoo_product_variant(self, woo_instance, product_sku, variant_id):
        ...
        if product_sku and not woo_product:
            ...
            _logger.info("elsewoo_product %s||2odoo_product %s", woo_product,odoo_product)
        _logger.info("2woo_product %s||2odoo_product %s", woo_product,odoo_product)
    ...
    def sync_products(self, product_data_queue_lines, woo_instance, skip_existing_products=False,
        ...
        for product_data_queue_line in product_data_queue_lines:
            ...
            _logger.info("woo_product_template_id %s", woo_product_template_id)
            _logger.info("woo_product_template_name %s",template_title)
            ...
            if data["variations"]:
                ...
                if new_woo_template:
                    ...
                    continue
            if data["type"] in ["woosb","simple","bundle"]:
                ...
                _logger.info("new_woo_template %s", new_woo_template)
    ...
    def simple_product_sync(self, woo_instance, product_response, product_queue_id, product_data_queue_line,
        ...
        _logger.info("woo_product %s||odoo_product %s", woo_product,odoo_product)
        if woo_product and not odoo_product:
            woo_template = woo_product.woo_template_id
            _logger.info("woo product id %s", woo_product)
            odoo_product = woo_product.product_id
            if skip_existing_products:
        ...
        if odoo_product:
            odoo_template = odoo_product.product_tmpl_id
            _logger.info("if odoo_product %s", odoo_template)
        ...
        if not woo_product:
            if not woo_template:
                ...
                _logger.info("woo_template_vals %s", woo_template_vals)
                if woo_template_vals.get("woo_product_type")=="woosb":
                   woo_template_vals.update({"woo_product_type":"simple"}) 
                woo_template = self.create(woo_template_vals)
    ...
    def update_stock(self, instance, export_stock_from_date):
        ...
        if not export_stock_from_date:
            export_stock_from_date = datetime.now() - timedelta(minutes=10)
        _logger.info("export_stock_from_date '%s'",export_stock_from_date)
        odoo_products = product_obj.get_products_based_on_movement_date_ept(export_stock_from_date,
                                                                            instance.company_id)


        _logger.info("odoo_products '%s'",odoo_products)
        instance.last_inventory_update_time = datetime.now()- timedelta(minutes=30)
        woo_templates = woo_product_product_obj.search([('product_id', 'in', odoo_products),
                                                        ('woo_is_manage_stock', '=', True)]).woo_template_id
        _logger.info("woo_templates1 '%s'",woo_templates)
        woo_templates = woo_templates.filtered(lambda x: x.woo_instance_id == instance and x.exported_in_woo)
        _logger.info("woo_templates 2 '%s'",woo_templates)
        odoo_temp = self.env['stock.move'].search([('exported_in_woo', '=',True),('product_id', 'in', odoo_products)])
        for rec in odoo_temp:
            rec.exported_in_woo = False
        if woo_templates:
